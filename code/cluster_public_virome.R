...
df <- read_tsv(
  "data/votu_match/my_clusters_pig.tsv",
...
shared_clusters <- df_long %>%
  group_by(cluster) %>%
  summarize(has_germany = any(is_germany), has_other = any(!is_germany)) %>%
  filter(has_germany & has_other) %>%
  pull(cluster)

shared_germany <- df_long %>%
  filter(cluster %in% shared_clusters, is_germany) %>%
  distinct(genome) %>%
  pull(genome)
shared_non_germany <- df_long %>%
  filter(cluster %in% shared_clusters, !is_germany) %>%
  distinct(genome) %>%
  pull(genome)
