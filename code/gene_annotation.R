...
pie_df <- annotation_df %>% 
  count(category) %>% 
  mutate(new_cat = if_else(category == "unknown function", "Unknown function", "Known function")) %>% 
  group_by(new_cat) %>% 
...
pie_plot <- pie_df %>% 
  ggplot(aes(ymax=ymax, ymin=ymin, xmax=3.5, xmin=2.5, fill=new_cat)) +
  geom_rect(color = "white") +
  geom_text(x=4.3, aes(y=labelPosition, label=label), size=4.5, fontface = "bold") +
  geom_text(x = 1.5,y =0.2, label ="Genes\n999.462", size = 6, fontface = "bold") +
  coord_polar(theta="y") + 
...
# Pie plot category ------------------------------------------------------------------------------------------
cat_colors <- c('black', '#ffffd9','#c7e9b4','#525252','#7fcdbb','#d9d9d9','#1d91c0',
                '#225ea8','#0c2c84')
...
pie_cat <- annotation_df %>%  
  ...
  filter(!category == "unknown function") %>% 
    group_by(category) %>% 
      ...
      mutate(ymax = cumsum(percent),
             ymin = c(0, head(ymax, n=-1)),
             category = case_when(category == "connector" ~ "Connector",
             category == "DNA, RNA and nucleotide metabolism" ~ "Nucleotide metabolism",
             category == "head and packaging" ~ "Head/Packaging",
             category == "integration and excision" ~ "Integration/Excision",
             category == "lysis" ~ "Lysis",
             category == "moron, auxiliary metabolic gene and host takeover" ~ "Auxiliary metabolic",
             category == "other" ~ "Other",
             category == "tail" ~ "Tail",
             category == "transcription regulation" ~ "Transcription"))

pie_cat$labelPosition <- (pie_cat$ymax + pie_cat$ymin) / 2
pie_cat$label <- paste0(pie_cat$category, "\n(", round(pie_cat$percent, 1),"%)")

pie_cat_plot <- pie_cat %>% 
  ggplot(aes(ymax=ymax, ymin=ymin, xmax=3.5, xmin=2.5, fill=category)) +
  geom_rect(color = "white") +
  geom_text(x=3.8, aes(y=labelPosition, label=label), size=3, fontface = "bold") +
  geom_text(x = 1.5,y =0.2, label ="Genes\n187.886", size = 6, fontface = "bold") +
  coord_polar(theta="y") + 
...
# Bar plot functions ------------------------------------------------------------
cat_colors_bars <- c('#ffffd9','#d9d9d9','#c7e9b4','#525252','#7fcdbb','#1d91c0',
                '#225ea8','#0c2c84')
...
barplot <- annotation_df %>% 
  ...
  ggplot(aes(x = percent,
             y = fct_reorder(annot, n),
             fill = category)) +
  geom_col() +
  geom_text(aes(x = percent +0.02,
                y = annot,
              label = annot),
            hjust = 0,
            size = 3.7) +
  ...
  scale_fill_manual(values = cat_colors_bars,
    labels = c(
...
ggsave(barplot, filename = "plots/figure4/barplot_genes.pdf", width = 3, height = 8, dpi = 300)
# Defense plot -----------------------------------------------------------------

defense_df <- read_tsv("data/representative_viral_defense_finder_systems.tsv") 

defense_trans <- defense_df %>% select(sys_beg, activity) %>% 
  mutate(sys_beg = str_remove(sys_beg, "_[0-9]+$")) %>% 
  count(sys_beg, activity) %>% 
  count(activity) %>% 
  mutate(percent = n /19778)

color_code <- c('#9e0142','#d53e4f','#f46d43','#fdae61','#fee08b','black' ,
                '#e6f598','#abdda4','#66c2a5','#3288bd','#5e4fa2','#8dd3c7',
                '#ffffb3','#bebada','#fb8072','#80b1d3','#fdb462','#b3de69',
                '#fccde5','#d9d9d9','#bc80bd','#ccebc5','#ffed6f' )


defense_plot <- defense_df %>% select(activity, type) %>% 
...
ggsave(defense_plot, filename = "plots/figure4/defense.pdf", width = 6, height = 5, dpi = 300)
...
ggsave(fig_4, filename = "plots/figure4/Figure_4.png",
...
ggsave(filename = "plots/gene_categories.png",
       width = 5,
       height = 5,
       dpi = 400)
...
ggsave(file = "plots/gene_annotation.png",
...
ggsave(file = "plots/gene_annotation_amg.png",
