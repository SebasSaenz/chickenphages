...
# Load files
clusters <- read_tsv(
  "data/my_clusters.tsv",
  col_names = c("cluster_representative", "cluster_members")
)
...
cluster_df |>
  ...
  ggplot(aes(y = checkv_quality, x = n)) +
  geom_col() +
  scale_x_continuous(limits = c(0, 9500), breaks = seq(0, 9500, 1000)) +
  scale_y_discrete(
    labels = c(
      "High-quality" = "High\nquality",
...
cluster_df |>
  ...
  ggplot(aes(y = checkv_quality, x = contig_length_mb)) +
  geom_boxplot(
    outlier.colour = "grey",
    outlier.shape = 1,
    staplewidth = 0.3,
...
cluster_df |>
  ...
  select(provirus) |>
  count(provirus) |>
  mutate(provirus = factor(provirus, levels = c("Yes", "No"))) |>
  ggplot(aes(x = provirus, y = n)) +
  geom_col(width = 0.5) +
  labs(y = "vOTUs", x = "Provirus") +
...
cluster_df |>
  ...
  ggplot(aes(x = viral_genes, y = checkv_quality)) +
  geom_boxplot(
    outlier.colour = "grey",
    outlier.shape = 1,
    staplewidth = 0.3,
...
clusters |>
  ...
  count(class) |>
  mutate(
    class = if_else(is.na(order), "Unclassified", class),
    percent = n / sum(n) * 100
  ) |>
  ggplot(aes(y = fct_reorder(class, n), x = n)) +
  geom_col() +
  scale_x_log10() +
  labs(y = "Viral classes", x = "vOTUs") +
  theme_bw() +
  theme(
    panel.grid.minor = element_blank(),
    panel.grid.major.y = element_blank(),
...
ggsave(
  file = "plots/suplementary/viral_classes.png",
  width = 6,
  height = 4,
  dpi = 400
)
...
clusters |>
  ...
  ggplot(aes(y = fct_reorder(order, n), x = n)) +
  geom_col() +
  scale_x_log10() +
  labs(y = "Viral order", x = "vOTUs") +
  theme_bw() +
  theme(
    panel.grid.minor = element_blank(),
    panel.grid.major.y = element_blank(),
...
clusters |>
  ...
  ggplot(aes(y = fct_reorder(family, n), x = n)) +
  geom_col() +
  scale_x_log10() +
  labs(y = "Viral family", x = "vOTUs") +
  theme_bw() +
  theme(
    panel.grid.minor = element_blank(),
    panel.grid.major.y = element_blank(),
...
llist_complete <- cluster_df %>%
  filter(checkv_quality == "Complete") %>%
  select(cluster_representative)

write_tsv(llist_complete, "data/list_complete_votus.txt")
