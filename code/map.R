...
labels <- inner_join(df, world, by = "region") %>%
  group_by(region) %>%
  summarise(avr_long = mean(long), avr_lat = mean(lat))
...
world %>%
  ...
  geom_text_repel(
    data = labels,
    aes(x = avr_long, y = avr_lat, label = region),
...
sex <- metadata |>
  ...
  mutate(
    host_gender = str_to_title(host_gender),
    host_gender = if_else(is.na(host_gender), "Unknown", host_gender)
  ) |>
  ggplot(aes(y = fct_reorder(host_gender, n), x = n, fill = study)) +
  geom_col(width = 0.5) +
...
git + sex + plot_layout(guides = 'collect') & theme(legend.position = "bottom")

ggsave(filename = "plots/sex_samples.png", width = 6.5, height = 3, dpi = 300)


x <- metadata |>
  filter(geo_loc_name_country == "Germany", grepl("UHO", Run)) |>
  count(Host_age)
