...
# taxonomy
tax_df <- host_df |>
  left_join(class_df, by = c("Virus" = "cluster_representative")) |> 
  ...
  mutate(
    ...
    class.y = if_else(is.na(class.y), "Unclassified", class.y)) |> 
  count(phylum.x, class.y) |> 
  inner_join(host_pool_df, by = c("phylum.x"= "phylum")) |> 
  mutate(phylum.x = if_else(host_pool, "Other", phylum.x),
         phylum.x = factor(phylum.x,
                         levels = phylum_factor)) |> 
  group_by(phylum.x) |> 
...
# Life style
 style_plot <- host_df |>
   ...
   mutate(
     ...
     phylum = str_remove(phylum, "_[A-Z]"),
     phylum = if_else(is.na(phylum), "Unknown", phylum),
     style = case_when(Virulent >= 0.95 ~ "Virulent",
                       Temperate >= 0.95 ~ "Temperate",
                       Temperate < 0.95 & Temperate < 0.95  ~ "Uncertain"),
     style = factor(style,
                    levels=c("Temperate", "Virulent", "Uncertain"))) |> 
   count(phylum, style) |> 
   inner_join(host_pool_df, by = "phylum") |> 
   mutate(phylum = if_else(host_pool, "Other", phylum),
          phylum = factor(phylum,
 ...
 ggsave(filename = "plots/figure3/Figure_3.png", width = 12, height = 6, dpi=400) 
...
ggsave(host_plot_genus, file = "plots/host_genus.png",
...
polyvalent_plot <- data.frame(tax_level = c("Genus", "Family", "Order"),
          value = c(5.601, 0.353, 0.055)) %>% 
  mutate(tax_level = factor(tax_level,
                            levels = c("Genus", "Family", "Order"))) %>% 
  ggplot(aes(x = tax_level,
             y = value)) +
  geom_col(width = 0.6) +
  geom_text(aes(x = tax_level, y = value + 0.2, 
...
ggsave(polyvalent_plot, file = "plots/polyvalent.png",
