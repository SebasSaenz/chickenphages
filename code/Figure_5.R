...
observed_diet_plot <- diversity_df %>% 
  ...
  labs(y = " Number of observed vOTUs",
       x = NULL) +
   theme_bw()
 ...
 library(MASS)
 glm_interaction_nb <- glm.nb(observed ~ strain * diet, data = observed_strain_plot)
 ...
 ggsave(filename = "plots/test_ileum.png",
...
# Diversity within samples -----------------------------------------------------
 ...
 shared_otus_table <- function(df) {
   ...
   for (i in seq_along(sample_pairs)) {
     ...
     results[[i]] <- data.frame(Sample1 = pair[1], Sample2 = pair[2], Shared_OTUs = shared_count)
   }
...
shared_diet_plot <- shared_otu_table %>% 
  ...
  mutate(result=if_else(diet.x == diet.y, "Same<br>diet", "Different<br>diet")) %>% 
...
shared_strain_plot <- shared_otu_table %>% 
  inner_join(strain_type, by=c("Sample1"="SampleID")) %>% 
  inner_join(strain_type, by=c("Sample2"="SampleID")) %>% 
  mutate(result=if_else(strain.x == strain.y, "Same<br>strain", "Different<br>strain")) %>% 
  ggplot(aes(x =result, y = Shared_OTUs)) +
  geom_boxplot(width =0.5) +
  scale_y_continuous(limits = c(0, 220)) +
  geom_line(data=tibble(x=c(1,2), 
...
glm_model <- glm(Shared_OTUs~result, data = shared_strain_plot, family = Gamma(link = "log"))
summary(glm_model)


observed_strain_plot + observed_diet_plot + shared_strain_plot + shared_diet_plot  + plot_layout(ncol = 2, axis_titles = "collect_y")
...
prevalent_votu_ile <- diversity_jaccard %>% 
  ...
  mutate(prevalence_LSL = LB < 108 & LSL>108,
         prevalence_LB = LB > 108 & LSL<108,
         prevalence =case_when(prevalence_LSL == TRUE & prevalence_LB == FALSE ~ "Prevalent in LSL",
...
illeum_abundance %>% 
  filter(genus == "Ligilactobacillus" | genus == "Limosilactobacillus" | genus == "Lactobacillus") %>% 
  ggplot(aes(x = strain,
             y = rel_abund)
         ) +
  geom_jitter(aes(color = genus),
              ...
              size = 0.8,
              shape = 21) +
  geom_boxplot(aes(group = interaction(strain, genus)),
               alpha = 0,
               outlier.alpha = 0,
               width = 0.7) +
...
ggsave(filename = "plots/strain_host_abundance.png",
       dpi = 300,
       width = 3,
       height = 3)
...
ggsave(filename = "plots/illeum_tax_lsl.png",
       dpi = 300,
       height = 3,
       width = 6)
# Ordination NMDS --------------------------------------------------------------
